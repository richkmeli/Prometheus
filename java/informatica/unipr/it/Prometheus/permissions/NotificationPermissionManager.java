package informatica.unipr.it.Prometheus.permissions;
...
public class NotificationPermissionManager {
    ...
    public boolean checkAppUsagePermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            UsageStatsManager usm = (UsageStatsManager) context.getSystemService(Context.USAGE_STATS_SERVICE); // permette l'accesso alle statistiche di utilizzo del dispositivo
            long time = System.currentTimeMillis(); // prende l'istante attuale
            List<UsageStats> appList = null; //  applist = pacchetto con le statistiche di ogni app in quel momento del sistema = queryUsageStats (intervallo(periodo) , tempo iniziale, tempo finale)
